<?php
/**
 * Blog sidebar — always the designed Voltalux sidebar (conversion CTA + recent
 * posts + categories + search). We intentionally do NOT fall back to WordPress'
 * default widget area here, so the blog always looks on-brand.
 *
 * @package Voltalux
 */
...
<aside class="vlx-sidebar" aria-label="<?php esc_attr_e( 'Zijbalk', 'voltalux' ); ?>">
	...
	if ( $recent->have_posts() ) :
		?>
		<div class="vlx-side-block">
			<h4 class="vlx-side-block__title"><?php esc_html_e( 'Recente artikelen', 'voltalux' ); ?></h4>
	...
	<?php $cats = wp_list_categories( array( 'echo' => false, 'title_li' => '', 'show_count' => true, 'hide_empty' => true ) ); ?>
	<?php if ( $cats ) : ?>
		<div class="vlx-side-block">
			<h4 class="vlx-side-block__title"><?php esc_html_e( 'Categorieën', 'voltalux' ); ?></h4>
			<ul class="vlx-side-cats"><?php echo $cats; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></ul>
		</div>
	<?php endif; ?>

	<div class="vlx-side-block">
		<h4 class="vlx-side-block__title"><?php esc_html_e( 'Zoeken', 'voltalux' ); ?></h4>
		<?php get_search_form(); ?>
	</div>
